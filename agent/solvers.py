...
If we can *prove* the answer with plain Python, we never touch a model at all.
Currently: arithmetic expressions and common percentage phrasings. Each solver
returns None unless it is confident, so a miss simply falls through to the
next tier (local model, then Fireworks) — a wrong free answer should be
...
_PERCENT_OF = re.compile(
    r"(?:what is\s+)?(\d+(?:\.\d+)?)\s*(?:%|percent)\s+of\s+(\d+(?:\.\d+)?)", re.I
)
_SQRT = re.compile(r"square root of\s+(\d+(?:\.\d+)?)", re.I)
...
def try_deterministic(category: str, prompt: str) -> str | None:
    if category == "math":
